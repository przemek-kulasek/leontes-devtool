...
## What to do
1. Implement step: can we communicate somehow with this dialog? maybe if we mention to update some file (mention it in spec) and also apart from free notes we can display this file or some other mechanism.
2. Spec & kickoff: maybe we should add info about catalog structure that should be used to let llm knows where we store materials etc. folders are listed but we should add location of images and whatever there is
3. Can we make project feature task delete more like in solution explorer in ficual studio instead of separate buttons?
4.
